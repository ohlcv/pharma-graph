...
```
用户在神经策略体系（pan/zoom 视野聚焦在神经策略节点）看图
未选中任何节点
点"开始漫游"
  → pickRoot() 返回 concept-exam-system（药考书）
  → 漫游突然跳到药考体系漫游
  → 用户体验："我明明在看神经策略，怎么跑了？"
```
...
|---|---|---|---|
| 选中药考节点 | 药考 | 漫游药考子树 | ✅ 正确 |
| 选中神经策略节点 | 神经策略 | 漫游神经策略子树 | ✅ 正确 |
| **不选节点** | **任意** | **总是从药考体系开始** | ⚠️ **不一致** |
...
### A.5 未来工作（不在本次 ARD 范围内）

| 优先级 | 任务 | 描述 |
|---|---|---|
| 高 | `pickRoot()` 体系感知 | 当无选中节点时，根据"当前 viewport 中心"或"最近查看的体系根"决定起点，而不是无脑返回 book-y2 |
| 中 | 跨体系节点 (`relates` 边) | 如果后续业务需要在两体系间建立"对比/关联"边（`relates` 边），需要扩展 `applyRootScope` 的 DOWN-only BFS 来支持"白名单+黑名单"的双向约束 |
| 中 | UI 提示"当前漫游体系" | 在漫游详情面板显示"漫游范围：药考体系 / 神经策略体系"，让用户清楚知道当前在哪个体系漫游 |
| 低 | 神经策略体系 fill 类型归一化 | `cls-feature` 在语义上接近 `cls-classification`（都是分类），可以考虑合并或重新映射到档位配置，让 L2 档位也能覆盖神经策略分类 |

### A.6 为什么不现在修 `pickRoot`

1. **当前只有一个用户实际看神经策略**（项目作者），且作者每次都会先选中节点再漫游，触发不到"无选中 + 神经策略"的场景
2. **修法需要先决定**：
   - 是"按 viewport 中心"（需要 cy 内核访问）？
   - 还是"按上次选中节点的最后所在体系"（需要 UI 状态记录）？
   - 还是"显式 prompt 让用户选"（打断式 UX）？
3. **业务优先级低**：神经策略体系的内容量 100 节点 vs 药考 762 节点，让用户先选中节点再漫游已是合理引导

未来当 `pickRoot()` 体系感知的需求出现时，建议在新的 ARD-005 中独立记录方案选择。
