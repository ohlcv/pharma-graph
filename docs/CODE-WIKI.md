...
- **职责**：边类型词汇表的 SSoT（issue #9）。导出 `EDGE_TYPES` readonly tuple（has/isa/activates/inhibits/mechanism/metabolizes/treats/causes/interacts/contraindicates/prerequisite/relates/sibling/contains/part_of/has_couplet/describes/specializes）、`EdgeType` 类型、`DEFAULT_EDGE_TYPE`（`'relates'`）、`isEdgeType()` 类型守卫。
