...
> 细分类节点的 tags **必须包含：自身 label + 子分类名（如有）+ 重点代表药**。
>
> **子分类名判定**：如果该分类有下一级子分类（如 SAMA/LAMA、SABA/LABA、ICS/LTRA），则必须包含这些子分类名。
...
# ✅ 短效M胆碱受体拮抗剂（SAMA）.md（strict-class）
tags:
  - SAMA               # 子分类名
  - 短效M胆碱受体拮抗剂  # 自身 label
  - 异丙托溴铵          # 重点代表药

# ✅ 吸入型肾上腺糖皮质激素（ICS）.md（strict-class，有子分类 ICS）
tags:
  - ICS                    # 子分类名
  - 吸入型肾上腺糖皮质激素   # 自身 label
  - 布地奈德                # 重点代表药
  - 氟替卡松                # 重点代表药

# ✅ 白三烯受体拮抗剂（LTRA）.md（strict-class，有子分类 LTRA）
tags:
  - LTRA               # 子分类名
  - 白三烯受体拮抗剂   # 自身 label
  - 孟鲁司特            # 重点代表药
...
**label 长度限制与换行规范**：
- label 长度建议控制在 **40 字符以内**，超过时应使用 YAML `|` block scalar 换行
- 换行位置：在句号（。）、逗号（，）或分号（；）后换行，保持语义完整
- 使用 `|` literal block scalar 保留换行，UI 展示时自动格式化
