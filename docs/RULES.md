...
> **重点药 vs 普通药**：均为 `fill: cls-drug`。重点药 `stroke: flow`（流光边框），普通药 `stroke: auto`（默认）。区分依据是教材里这张药**是不是被"画像"过**。
...
| `auto` | — | 由 fill 决定 | 留空走 fill 默认（可为 vee / tag / barrel 等扩展形状）|
...
> 1. 节点显式 `stroke`（flow/glow）→ `STROKE_CONFIG[stroke].color`
> 2. 节点没填 `stroke` → 用 `FILL_CONFIG[fill].defaultStroke`（fill 自带默认边框风格）
> 3. 节点填了 `stroke="auto"` 或没 fill → 走 subtreeRoot 色 / depth 灰阶 fallback

### 3.1 fill 字段定义

> **"默认 stroke"** = 节点不填 stroke 时使用 `FILL_CONFIG[fill].defaultStroke` 的值（详见 3.4）。
> **"stroke=auto 边框色（兜底）"** = 节点显式 `stroke: auto` 且无 subtreeRoot 时，按 fill 取的边框色（参与渲染，见 4.1 第 4 步）。

| fill | 中文含义 | 形状 | 背景色 | 默认 stroke | stroke=auto 边框色（兜底） |
|---|---|---|---|---|---|
...
| `stroke` | 用用户填的 stroke（含 `'auto'` / `'fallback'`） | `FILL_CONFIG[fill].defaultStroke`（fill 缺失时用 `'auto'`） |

**关键点**：
- `fill` 是**推荐字段**而非必填——不填时节点仍可渲染（通用外观），但失去 fill 提供的语义色
- `shape` 不填 → 用 `FILL_CONFIG[fill].shape`（每个 fill 都有自己的形状）
- `stroke` 不填 → 用 `FILL_CONFIG[fill].defaultStroke`
- 用户填 `stroke="auto"` 就是显式表达"我要 auto"，**不会被 fill.defaultStroke 覆盖**（经典"显式值 vs 默认值"模式）
- 一旦用户表达 `stroke`，`fill.defaultStroke` 自动让位、不参与决策

**示例**：
- `fill: cls-drug` （无 shape/stroke）→ 椭圆药物 + 流光边框（fill 兜底）
- `fill: cls-drug, shape: diamond` → 菱形药物 + 流光边框（shape 显式，stroke 仍走 fill 兜底）
- `fill: cls-drug, stroke: auto` → 椭圆药物 + 中性边框（stroke 显式 auto 覆盖 fill.defaultStroke=flow）
...
|---|---|---|
| `cls-drug` | `flow` | 药物是图谱核心，重点药需要流光动画强调 |
| `cls-summary` | `glow` | 总结节点跨章节，光晕更醒目 |
| 其他 fill | `auto` | 跟随子树色（subtreeRoot），无子树时降级到 fill 兜底（按 fill 类型着色，比中性灰更直观） |

如需某 fill 始终按 fill 颜色着色（不跟子树走），可把 `defaultStroke` 改为 `fallback`。

### 3.5 边框色配置

- `FILL_BORDER_HINTS`（在 `src/core/config.ts` 末尾）：**fill 兜底边框色**，stroke=auto 链路无 subtreeRoot 时（4.1 第 4 步）和 stroke=fallback 时（4.1 第 2 步）都用它
...
> - 显式填写 `stroke: flow` → 覆盖 fill 的默认边框（彩色流光）
> - 显式填写 `stroke: glow` → 覆盖 fill 的默认边框（多层光晕）
> - 不填或填 `auto` → 使用 fill 的默认配置
>
> 药学含义：`cls-drug`（药物）默认是椭圆；显式填 `shape: diamond` 可让某个药物用菱形表示（如作为"重点药"区分）。
...
| `flow` | subtreeRoot 色 | solid 实线 | 彩色流光 |
| `glow` | subtreeRoot 色 | solid 实线 | 三层光晕 |
...
- `fallback`：**永远**按 fill 兜底色着色，subtreeRoot 完全不参与（适合没有子树归属感的节点，如结构/概念）
- 何时用哪个：`FILL_CONFIG[fill].defaultStroke` 配置——按 fill 类型语义决定（详见 3.4）
