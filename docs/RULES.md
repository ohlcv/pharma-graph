...
> **构图原则（初学搭建框架阶段）：上层维持严格树状层级以建立清晰知识骨架，仅允许在底层叶子节点局部引入网状横向关联，不追求一次性还原现实全部复杂联系。**
> 原理依据：现实知识本体是完整网状系统，但人的工作记忆有限；树状结构保障框架可理解、可维护，网状用于捕捉跨节点的隐性联系。采用“上树下网”的混合认知模型，本体可留存全部关系，可视化视图做适度裁剪，避免因过度还原真实而陷入信息过载、视图混乱。

本项目采用 OWL2 风格的 **6 种边关系类型** + **三维度视觉编码**：

- **shape**（OWL2 实体类型）：显式填写时覆盖 fill 的默认形状（一对一固定映射）
- **fill**（领域顶层类）：决定背景色 + 默认形状 + 默认边框色（配置中心）
- **stroke**（边框样式组合值）：显式填写时覆盖 fill 的默认边框（双线/光晕/流光）

> **fill 是配置中心**：每个药学顶层类定义完整默认外观；shape/stroke 显式填写时覆盖 fill 默认值。
> **double 是重点节点专用**：所有 fill 的 `defaultStroke` 统一为 `glow`（默认呼吸光晕）；重点节点需在节点 frontmatter 中显式写 `stroke: double`。
...
- md 填了 stroke 时，在 defaultStroke 之上**叠加** md 自己的效果：边框本体由 md 决定，覆盖层特效两者并存
...
- `fill: cls-drug, stroke: double` → 椭圆药物 + 双线边框（重点药/重点分类）
...
>
> 如需某 fill 始终按 fill 颜色着色（不跟子树走），可把 `defaultStroke` 改为 `fallback` 或让节点显式写 `stroke: fallback`（需同步改 `src/core/config.ts`）。
...
**边框本体（线型/颜色）取色优先级**（覆盖层特效独立叠加，见下文）：
...
6. 用户填 `stroke: auto` + 无 subtreeRoot → `FILL_BORDER_HINTS[fill]`
7. 用户不填 stroke → `FILL_CONFIG[fill].defaultStroke`（统一为 `glow`）
8. 节点连 fill 都没填 → `FILL_BORDER_DEFAULT`（中性灰）

> **特效叠加（与上面独立）**：`defaultStroke` 的 glow/flow 特效**始终生效**；md 填的 `stroke` 特效在此基础上再叠加。
