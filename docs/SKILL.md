...
- tags 从 summary 加粗词抽取、语义去重、不写 essence 枚举值（详见「tags 规范速查」）：
  ...
  - **细分类节点**（strict-class）：自身 label + 机制/分类依据 + 重点代表药（普通药不标记）
...
| 细分类节点（strict-class） | 自身 label + 机制/分类依据 + 重点代表药（普通药不标记） | 如 `短效β₂受体激动剂（SABA）, 激动β₂受体, 特布他林, 沙丁胺醇` |
| 口诀节点 | `口诀` + 药品名/分类名 | `口诀, 乙酰半胱氨酸`（label 改为口诀内容） |

> **分类节点核心原则**：必须包含自身 label；无子分类的细分类节点应包含机制（如 `激动β₂受体`），不应重复已在 label 中的缩写（如 SAMA）。详见 RULES §5.11.4.6.3。
