...
##### 5.11.4.5 药品节点 tags 规范

> 药品节点的 tags **必须包含药品名**，用于图谱检索和分类归属。
...
# 错误：药品名不在 tags 里（无法通过"氨溴索"检索到该节点）
tags:
  - 黏痰溶解剂
```

##### 5.11.4.6 分类节点 tags 规范（按节点类型区分）

> 分类节点（`strict-class` / `umbrella-class`）的 tags 规范如下：
>
> **核心原则**：
> 1. **必须包含自身 label**（如 `黏痰溶解剂`、`M胆碱受体拮抗剂`）
> 2. **必须包含子分类名**（如 SAMA、LAMA、SABA、LABA、ICS、LTRA）
> 3. **仅标记重点药**（普通药不标记）

###### 5.11.4.6.1 模块节点（module）

模块节点（篇、章、节入口）一般不单独建 tags，若有 tag 需求，按具体内容抽取。
...
###### 5.11.4.6.2 粗分类节点（umbrella-class）

> 粗分类节点的 tags **必须包含自身 label + 子分类名 + 重点代表药**。
>
> 注意：粗分类节点的 label 通常是伞形大类（如"扩张支气管"），其下的子分类节点会包含更细的分类。

```yaml
# 扩张支气管.md（umbrella-class）
tags:
  - 扩张支气管          # 自身 label
  - β₂受体激动剂        # 子分类名
  - M胆碱受体拮抗剂     # 子分类名
...
###### 5.11.4.6.3 细分类节点（strict-class）

> **细分类节点**的 tags **必须包含：自身 label + 机制/分类依据 + 重点代表药**。
>
> **规范说明**：
> 1. **自身 label**：与 label 完全一致
> 2. **机制/分类依据**：药理作用或分类标准（如 `抑制5-HT和NE再摄取`、`激动β₂受体`）
>    - 若有子分类（如 SAMA/LAMA），子分类名**不应**单独列出（已被 label 包含缩写）
> 3. **重点代表药**：每类选 1-2 个代表药
>
> **冗余检查**：若 subcategory 缩写（如 SAMA）已包含在 label 中（如「短效M胆碱受体拮抗剂（SAMA）」），只列机制如 `拮抗M胆碱受体`，不必再列 `SAMA`。

```yaml
# ✅ M胆碱受体拮抗剂.md（strict-class，有子分类 SAMA/LAMA）
tags:
  - M胆碱受体拮抗剂      # 自身 label
  - SAMA                # 子分类名
  - LAMA                # 子分类名
...
# ✅ β₂受体激动剂.md（strict-class，有子分类 SABA/LABA）
tags:
  - β₂受体激动剂         # 自身 label
  - SABA               # 子分类名
  - LABA               # 子分类名
...
# ✅ 短效M胆碱受体拮抗剂（SAMA）.md（strict-class，无子分类）
tags:
  - 短效M胆碱受体拮抗剂（SAMA）  # 自身 label（包含 SAMA）
  - 拮抗M胆碱受体                # 机制/分类依据
  - 异丙托溴铵                  # 重点代表药

# ✅ 短效β₂受体激动剂（SABA）.md（strict-class，无子分类）
tags:
  - 短效β₂受体激动剂（SABA）  # 自身 label（包含 SABA）
  - 激动β₂受体                # 机制/分类依据
  - 特布他林                   # 重点代表药
  - 沙丁胺醇                   # 重点代表药

# ✅ 吸入型肾上腺糖皮质激素（ICS）.md（strict-class，无子分类）
tags:
  - 吸入型肾上腺糖皮质激素（ICS）  # 自身 label（包含 ICS）
  - 抗炎                            # 机制/分类依据
  - 布地奈德                        # 重点代表药
  - 氟替卡松                        # 重点代表药

# ✅ 白三烯受体拮抗剂（LTRA）.md（strict-class，无子分类）
tags:
  - 白三烯受体拮抗剂（LTRA）  # 自身 label（包含 LTRA）
  - 拮抗白三烯受体            # 机制/分类依据
  - 孟鲁司特                  # 重点代表药

# ✅ 三环类抗抑郁药（TCAs）.md（strict-class，无子分类）
tags:
  - 三环类抗抑郁药（TCAs）  # 自身 label（包含 TCAs）
  - 抑制5-HT和NE再摄取      # 机制/分类依据
  - 阿米替林                 # 重点代表药
  - 氯米帕明                 # 重点代表药

# ✅ 恶心性和刺激性祛痰药.md（strict-class，无子分类）
tags:
  - 恶心性和刺激性祛痰药  # 自身 label
  - 愈创甘油醚            # 重点代表药
  - 氯化铵                # 重点代表药

# ✅ 黏痰溶解剂.md（strict-class，无子分类）
tags:
  - 黏痰溶解剂            # 自身 label
  - 溴己新                # 重点代表药
  - 氨溴索                # 重点代表药
...
###### 5.11.4.6.4 代码层面的 label 过滤

> **重要**：代码会自动过滤与 label 重复的 tag，因此 md 中的 tags 可以包含与 label 相同的值，UI 显示时会自动隐藏重复部分。
>
> 实现位置：`src/ui/detail-panel.ts` 的 `buildHeroHtml()` 函数。
...
这意味着：分类节点的 tags 里包含自身 label 是允许的，代码会自动处理重复显示问题。

###### 5.11.4.6.5 判断锚点

| 问自己 | 答案 |
| --- | --- |
| tags 里是否包含自身 label？ | 是 → 正确（代码会自动过滤） |
| 是否有子分类（SAMA/LAMA/SABA/LABA/ICS/LTRA）？ | 是 → 必须包含子分类名 |
| 药品名是否全写了？ | 否 → 只写重点药，普通药不写 |

##### 5.11.4.7 口诀节点 tags 与 label 规范

> 口诀节点（`essence: mnemonic`）的 label 应改为口诀内容，tags 包含 `口诀` 和对应的药品名/分类名。
...
##### 5.11.4.8 tags 显示过滤规则（UI 层面）

> 为节省界面空间，UI 渲染时自动隐藏以下重复标签：
> - **规则 1**：如果 tag 与 nodeName（label）相同 → 隐藏
> - **规则 2**：如果节点是口诀（`essence === 'mnemonic'`）且 tag 是 `口诀` → 隐藏
...
**效果**：
- 药品节点 `氨溴索` 的 tags 里有 `氨溴索`，显示时自动隐藏，避免标签区出现重复的"氨溴索"
- 口诀节点 `半胱氨酸真优秀，祛痰解毒666...` 的 tags 里有 `口诀`，显示时自动隐藏

> **重要**：MD 文档内的 tags 保持不变，只是显示时隐藏重复部分。这意味着通过 API/数据导出的 tags 仍是完整的。
