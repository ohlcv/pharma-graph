...
> ⚠️ **ARCHIVED — 2026-07-14**：本文档是 ISA 迁移**前**生成的违规清单快照。所有 375 处"has→location 子级"违规已由 [`archive/scripts/migrate-isa.ts`](../../archive/scripts/migrate-isa.ts) 修复完毕（has 边从 384 降至 0）。当前 frontmatter 校验通过状态见 [migration-report.md](../migration-report.md)。本文档**不再代表当前数据状态**。
