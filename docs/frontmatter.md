...
1. **先扫 `summary.short` / `summary.full`**，抽出所有加粗短语、专有名词、药理分类词。
2. **对候选词做语义去重**——检查是否存在同/近义词同时出现，保留先出现的，丢弃重复的。
3. **只有当 summary 里完全找不到合适关键词时**才允许自取（须基于 `summary` 上下文，不引入 summary 没有的概念）。
4. **最后用 §4 规则 4–8 的硬约束再过滤一遍**（不重复 essence/level/tier、不写本质词、不写整句、控制数量）。
