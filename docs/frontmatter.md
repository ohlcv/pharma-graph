...
## 2.2 ~~tier — 知识层级（决定填充色）~~（已废弃）

> **背景**：早期版本用 `tier` 表达"药学知识自然层级"（basic / drug / disease / management / service / legal 六层），但在 215 个有 tier 的 md 中全部填写 `tier: drug`，导致概念失效、视觉全同色。
>
> **现状**（2026-09）：
> - frontmatter **不再解析** `tier` 字段（向后兼容：旧字段被忽略，不再校验）
> - 节点填充色由 **`essence` 直接推导**，每个 essence 配独立语义色（互不相同）
>
> **9 种 essence 颜色映射**（一一对应、有意义）：
>
...
> | `medication` | 具体药物/制剂 | 宝石蓝 `#60a5fa` | `#2563eb` | 与 module 同色系明暗区分 |
...
>
> **核心原则**：
> 1. **颜色互不相同**：9 种 essence 9 种色，绝不复用
> 2. **颜色有意义**：能直觉联想到该 essence 的语义
> > 4. **填充色不得承担"重点/难点/掌握状态"等其他含义**
>
> **迁移说明**：
> - 现有 md 中的 `tier:` 字段会被自动忽略，不报错
> - 不需要人工删除 `tier:` 行（保留也无害）
> - 如需批量清理，可运行：`grep -rlZ '^\s*tier:' public/content/ | xargs -0 sed -i '/^\s*tier:/d'`（macOS 因 BSD sed 不支持文件名过长，推荐用 Python 脚本遍历）
