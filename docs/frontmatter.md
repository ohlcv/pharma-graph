...
辅助节点（`mnemonic` 口诀、`summary` 总结、`table` 表格、`note` 注意）的 `edges_out` 也用 `part_of` 指向它们所要解释/记忆/辅助的主知识节点。
...
1. 是 `instance_of + reason: 治疗` 的目标；
2. 是 `instance_of + reason: 禁忌` 的目标；
3. 是多个节点共同关联的重要疾病；
4. 原纸图已经把该疾病单独画成一个方框；
5. 该疾病是后续多个关系的汇聚节点。
...
├─ 填充：tier drug        → 淡蓝色药物层
│
├─ subclass_of  蓝色实线箭头   → 子类→父类（类-类）
├─ part_of      绿色实线箭头   → 局部→整体 / 辅助节点→主知识
├─ instance_of  橙色实线箭头   → 实例→类别（个体-类）
├─ disjoint_with 紫色虚线     → 互斥（对称）
└─ equivalent_to  灰紫点线     → 等价（对称）
```
...
8. `edges_out.type` 只能使用 5 种正式边类型（subclass_of / part_of / instance_of / disjoint_with / equivalent_to）；
9. 所有具体关系词写在 `reason`，不自行创造新的 `type`；
10. `edges_out` 不允许父节点反向枚举子节点；
11. 不允许同一结构关系双向重复声明；
      ...
      essence              level                 tier
        ...
        形状               边框色               填充色
          ...
          └───────────────────┼───────────────────┘
           ...
           ┌───────────┬─────────┼─────────┬───────────┐
           ↓           ↓         ↓         ↓           ↓
        subclass_of  part_of  instance_of  disjoint_with  equivalent_to
           │           │         │           │              │
          类-类       局部-整体   个体-类       互斥（对称）    等价（对称）
                              │
                           reason
                              │
                    具体中文关系语义
