...
| 节点类型 | 必须包含 | 说明 |
|---|---|---|
| **药物节点**（drug / medication） | 药品名 + 分类 tag + 作用 tag | 分类如 `H₁受体拮抗剂`、`他汀类`；作用如 `扩张血管`、`降压`；可另加 ADR/禁忌/注意事项 tag |
| **模块节点**（module） | 一般不单独建 tags | 篇、章、节入口等结构节点 |
| **粗分类节点**（umbrella-class） | 自身 label + 子分类名 + 每子类 1 个重点代表药 | 如 `扩张支气管` + `β₂受体激动剂`、`M胆碱受体拮抗剂`、`黄嘌呤类药物` + 沙丁胺醇、异丙托溴铵、茶碱 |
| **细分类节点**（strict-class） | 自身 label + 机制/分类依据 + 重点代表药 | 普通药不标记；如 `短效β₂受体激动剂（SABA）` + `激动β₂受体` + 沙丁胺醇、特布他林 |
