...
1. 标签必须是有检索价值的关键词。
2. 不把 `essence`、`level`、`tier` 的枚举值重复写入 `tags`。
3. 不把“药物/疾病/概念”等节点本质重复写成标签。
4. 不把整个句子放进 `tags`。
5. 建议每个普通知识节点 2–8 个；不是硬性要求，纸图迁移时以原资料信息完整性优先。
6. 疾病如果已经建立独立 `illness` 节点，仍允许在相关药物节点保留疾病名标签，作为检索索引。
