---
data:
  id: jak-inhibitors
  label: JAK 激酶抑制剂
  essence: module
  field: pharmacology
  tier: drug
  location:
    book: 药学专业知识二
    chapter: 第二章 解热、镇痛、抗炎、抗风湿及抗痛风药
    section: 第二节 抗风湿药
  tags:
    - tsDMARDs
    - JAK
    - 靶向合成
  summary:
    short: 口服小分子靶向药，抑制 JAK 激酶阻断炎症信号通路，用于类风湿关节炎等，不良反应包括感染和血栓风险。
    full: JAK 激酶抑制剂是口服小分子靶向合成 DMARDs（tsDMARDs），通过抑制 JAK 激酶阻断细胞因子信号通路而发挥抗炎作用。代表药：托法替布、巴瑞替尼。用于类风湿关节炎、银屑病关节炎等。不良反应：感染风险升高（用药前需筛查结核/乙肝）、血栓风险升高（需关注深静脉血栓和肺栓塞）、血脂升高、肝功能异常。活动性感染、血栓病史患者慎用。
  edges_out:
    - target: biodmards
      type: isa
      reason: JAK 激酶抑制剂属于 tsDMARDs，是 bDMARDs/tsDMARDs 的重要组成
---

# JAK 激酶抑制剂

## 它是什么药？

属于**靶向合成DMARDs（tsDMARDs）**，通过**抑制JAK激酶**阻断细胞因子信号通路而发挥抗炎作用。**口服小分子**，使用方便。

## 代表药有哪些？

| 药物 | 特点 |
|---|---|
| **托法替布** | JAK1/JAK3抑制剂 |
| **巴瑞替尼** | JAK1/JAK2抑制剂 |

## 临床应用是什么？

- 类风湿关节炎
- 银屑病关节炎

## 典型不良反应是什么？

- 感染风险升高（用药前需筛查结核/乙肝）
- **血栓风险升高**：深静脉血栓、肺栓塞（需关注）
- 血脂升高
- 肝功能异常

## 禁忌人群是什么？

- 活动性感染
- 血栓病史患者慎用

## 它和章的关系？

它是tsDMARDs代表，定位"口服小分子JAK激酶阻断剂，感染+血栓风险是主要关注点"。
